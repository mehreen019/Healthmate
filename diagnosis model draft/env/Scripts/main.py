...
for i in range(len(df_tt)):
    y_symp.append(df_tt.columns[df_tt.iloc[i] == 1].to_list())
    y_disease.append(df_tt.iloc[i, -1])

# # I- GET ALL SYMPTOMS

all_symp_col = list(df_tr.columns[:-1])
all_symp_col_y = list(df_tt.columns[:-1])
...
all_symp_y = [clean_symp(sym) for sym in (all_symp_col_y)]

scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(all_symp)
X_test_scaled = scaler.transform(all_symp_y)
...
knn.fit(X_train_scaled, disease)
predData = knn.predict(X_test_scaled)
accuracy = accuracy_score(y_disease, predData)
...
while True:
            ...
            if suggestions==[]:
                ...
                if(len(suggestions)>0):
                    ...
                    for i in range(len(suggestions)):
                        ...
                        ans = input(msg)
                        if(ans=="yes"):
                            temp1[1]=1           #similarity 1 cause this is what they're experiencing
                            temp1[2]=suggestions[0]
                            suggestions=[]
                            break
            ...
            if suggestions==[]:
                ...
                if(len(suggestions)>0):
                    ...
                    for i in range(len(suggestions)):
                        ...
                        ans = input(msg)
                        if(ans=="yes"):
                            temp2[1]=1           #similarity 1 cause this is what they're experiencing
                            temp2[2]=suggestions[0]
                            suggestions=[]
                            break
    ...
    if current_state=="possible_disease":
        ...
        if userAllSymp==[]:
            userAllSymp=[col_dict[temp1[2]], col_dict[temp2[2]]]
    ...
    if current_state=="predict":
        result = knn.predict(userAllSymp)
        current_state = "END"
    ...
    if current_state=="END":
        ...
        if result is not None:
            ...
            maindis = result[0]
            print("you may have " + result[0] + ". Tap D to get a description of the disease .")
        else:
